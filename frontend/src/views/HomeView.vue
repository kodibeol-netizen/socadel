...
export default {
  ...
  data() {
    return {
      ...
      enCours: false,
      redirectionEnCours: false, // 🔒 Protège l'interface contre les interférences du bloc onerror
      messageStatut: 'Prêt',
      progression: 0,
      logs: [],
      fluxEvenements: null
  ...
  methods: {
    demarrerTraitement() {
      ...
      this.enCours = true;
      this.redirectionEnCours = false; // Réinitialisation du drapeau à chaque démarrage
      this.progression = 0;
      ...
      // Écoute des messages envoyés en temps réel par le serveur
      this.fluxEvenements.onmessage = (event) => {
        try {
          const donnees = JSON.parse(event.data);

          // 1. Cas d'erreur renvoyée par le serveur Python
          if (donnees.erreur) {
          ...
          // 2. Cas de mise à jour textuelle du statut
          if (donnees.statut) {
            this.messageStatut = donnees.statut;
            this.ajouterLog(`ℹ️ ${donnees.statut}`);
          }

          // 3. Cas de mise à jour de la barre de progression globale
          if (donnees.progression !== undefined) {
            this.progression = parseInt(donnees.progression, 10);
          }

          // 🔥 CAS A : RECHARGEMENT DE CONTRÔLE (Fin de téléchargement sur la même date)
          if (donnees.action === 'rechargement') {
            this.ajouterLog(`🔄 Fin des téléchargements. Lancement de la réactualisation de contrôle...`);
            
            this.redirectionEnCours = true; // Active la protection anti-clignotement
            this.arreterFlux();
            
            // Relance automatique sur la même date après 2 secondes pour valider l'intégrité
            setTimeout(() => { 
          ...
          // 🔥 CAS B : REDIRECTION AUTOMATIQUE (Total == 0, passage au jour précédent)
          if (donnees.action === 'redirection' && donnees.prochaine_date) {
            this.ajouterLog(`🎉 Vérification validée avec succès. Passage automatique au : ${donnees.prochaine_date}`);
            
            this.redirectionEnCours = true; // Active la protection anti-clignotement
            this.dateCible = donnees.prochaine_date; // On recule d'une journée
            this.arreterFlux();
            
            // Lancement de la nouvelle journée après 2 secondes
            setTimeout(() => { 
              this.demarrerTraitement(); 
            }, 2000);
            return;
          ...
          // Si le message n'est pas du JSON (texte brut)
      ...
      // Interception des déconnexions ou de la fermeture naturelle du script
      this.fluxEvenements.onerror = () => {
        // Sécurité : On ignore l'extinction si l'interface est en phase de transition planifiée
        if (this.redirectionEnCours) {
    ...
    arreterFlux() {
      ...
      // On maintient l'état d'animation actif si le serveur bascule d'une date à l'autre
      if (!this.redirectionEnCours) {
        this.enCours = false;
      }
    },
    ...
    ajouterLog(texte) {
      ...
      // Maintient l'ascenseur de défilement de la console bleue vers le bas
      this.$nextTick(() => {
